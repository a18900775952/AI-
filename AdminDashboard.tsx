...
import { 
  ...
  PencilIcon, CommandLineIcon, LightBulbIcon, SparklesIcon
...
export default function AdminDashboard({ onClose, onUpdate, user, t }: any) {
  ...
  const [newlyLearnedItems, setNewlyLearnedItems] = useState<InternalReference[]>([]);
      ...
      return (
        <div className={`grid gap-4 ${type === 'numeric' ? 'grid-cols-1 md:grid-cols-2 lg:grid-cols-3' : 'grid-cols-1'}`}>
            {targetFields.map(field => {
                if (type === 'numeric') {
                    // Update label to indicate the calculation split logic
                    const unitLabel = field.key === 'asset_total_m' ? 'Unit (1M)' : field.key === 'currency_havoc_w' ? 'Unit (1W)' : 'Unit';
                    return (
                        <div key={field.key} className="bg-void-800 p-4 rounded-lg border border-white/5">
                            ...
                            <div className="flex items-center gap-2"><span className="text-sm font-bold text-white">¥</span><input type="number" step="0.01" value={matrix?.rates[field.key] || 0} onChange={e => updateMatrixRate(field.key, parseFloat(e.target.value))} className="bg-transparent text-lg font-mono text-emerald-400 outline-none w-24 border-b border-white/10 focus:border-emerald-500" /><span className="text-xs text-gray-500">/ {unitLabel}</span></div>
                            {/* Visual Hint for Split Logic */}
                            {(field.key === 'asset_total_m' || field.key === 'currency_havoc_w') && (
                                <div className="text-[9px] text-primary/50 mt-1 flex items-center gap-1">
                                    <ScaleIcon className="w-3 h-3"/> AI Weight: Split by {field.key === 'asset_total_m' ? '10M' : '200W'}
                                </div>
                            )}
                        </div>
                    );
                } else if (field.key === 'collection_weapon') {
                    // Explicitly Render Quality Variants for Collection Weapons
                    return (
                        <div key={field.key} className="mb-4">
                            <h5 className="text-xs text-gray-300 font-bold mb-2 flex items-center gap-2"><SparklesIcon className="w-4 h-4 text-accent-gold"/> {field.label} (Quality Breakdown)</h5>
                            <div className="space-y-1 pl-1 border-l border-white/5 max-h-96 overflow-y-auto custom-scrollbar">
                                {(field.options || []).map(opt => (
                                    <div key={opt} className="mb-2">
                                        <div className="text-[10px] text-primary/70 font-bold mb-1 px-2">{opt}</div>
                                        {DELTA_QUALITIES.map(q => {
                                            const variantKey = `${opt}(${q})`;
                                            const rateKey = `${field.key}:${variantKey}`;
                                            return (
                                                <div key={rateKey} className="flex justify-between items-center text-xs border-b border-white/5 pb-1 pl-4 hover:bg-white/5">
                                                    <span className={`truncate w-32 ${q.includes('S') ? 'text-amber-400 font-bold' : 'text-gray-400'}`}>{q}</span>
                                                    <div className="flex items-center"><span className="text-gray-600 mr-1">¥</span><input type="number" value={matrix?.rates[rateKey] || 0} onChange={e => updateMatrixRate(rateKey, parseFloat(e.target.value))} className={`w-20 bg-transparent text-right font-mono outline-none focus:text-primary ${q.includes('S') ? 'text-amber-400' : 'text-white'}`} /></div>
                                                </div>
                                            )
                                        })}
                                    </div>
                    ...
                    // Standard Option Renderer (Infrastructure & others)
                    return (
                        <div key={field.key} className="mb-4">
                            ...
                            <div className="space-y-1 pl-1 border-l border-white/5 max-h-60 overflow-y-auto custom-scrollbar">
                                {(field.options || []).map(opt => {
                                    ...
                                    // Highlight if it's a max level item
                                    const isMax = opt.includes('满级') || opt.includes('Lv.10') || opt.includes('3x3');
                                    return ( 
                                        <div key={opt} className="flex justify-between items-center text-xs border-b border-white/5 pb-1">
                                            <span className={`truncate w-48 ${isMax ? 'text-primary font-bold' : 'text-gray-400'}`} title={opt}>
                                                {isMax && <span className="mr-1 text-[9px] bg-primary/20 px-1 rounded">MAX</span>}
                                                {opt}
                                            </span>
                                            <div className="flex items-center"><span className="text-gray-600 mr-1">¥</span><input type="number" value={matrix?.rates[rateKey] || 0} onChange={e => updateMatrixRate(rateKey, parseFloat(e.target.value))} className="w-16 bg-transparent text-right font-mono text-white outline-none focus:text-primary" /></div>
                                        </div> 
                                    );
                                })}
                            </div>
